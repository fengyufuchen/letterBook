...
<!--分页的form-->
<form id="pagerForm" action="admin/adminBook/getmanagerBookPage.action" method="post" >
</form>		

<div class="pageHeader">
				
<!--查询的form-->
<form id="searchClass" rel="pagerForm" onsubmit="return navTabSearch(this);" action="admin/adminBook/getmanagerBookPage.action" method="post">
...
<div class="pageContent">
	<form id="deletebook" rel="pagerForm" onsubmit="return validateCallback(this, dialogAjaxDone);" action="admin/adminBook/deleteBook.action" method="post">
	<div class="panelBar">
		<ul class="toolBar">
			<li >
				<a class="edit" mask="true" target="dialog" fresh="true" href="admin/adminBook/editBook.action?boid={boid}" warn="请选择一个图书!">
					<span>查看详情</span>
				</a>
			</li>
			<li>
			...
			<li >
				<a class="icon" href="admin/adminBook/getmanagerBookPage.action" target="navTab" fresh="true" rel="bookManager" title="图书管理">
					<span>刷新</span>
				</a>
			</li>
		</ul>
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="11%" align="center" orderField="boname" <c:if test="${orderField=='boname' }">class="${orderDirection}"</c:if> >书名</th>
				<th width="11%" align="center" orderField="boauthor" <c:if test="${orderField=='boauthor' }">class="${orderDirection}"</c:if> >作者</th>
				<th width="11%" align="center" orderField="boprice" <c:if test="${orderField=='boprice' }">class="${orderDirection}"</c:if> >价格</th>
				<th width="11%" align="center" orderField="bopress" <c:if test="${orderField=='bopress' }">class="${orderDirection}"</c:if> >出版社</th>
				<th width="14%" align="center" orderField="botime" <c:if test="${orderField=='botime' }">class="${orderDirection}"</c:if> >出版时间</th>
				<th width="11%" align="center" orderField="bosummary" <c:if test="${orderField=='bosummary' }">class="${orderDirection}"</c:if> >简介</th>
				<th width="5%" align="center" orderField="bonumber" <c:if test="${orderField=='bonumber' }">class="${orderDirection}"</c:if> >总数量</th>
				<th width="5%" align="center" orderField="boamount" <c:if test="${orderField=='boamount' }">class="${orderDirection}"</c:if> >剩余数量</th>
				<th width="11%" align="center">所属商家</th>
		...
		<tbody>
			<c:forEach items="${page.content}" var="bo">
				<tr target="boid" rel="${bo.boid}">
					...
					<td align="center">${bo.business.bname }</td>
					<td align="center">
						<c:choose>
