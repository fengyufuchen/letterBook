...
KE.setting = {
	...
	endlineTags : [
		'br', 'hr', 'table', 'tbody', 'td', 'tr', 'th', 'div', 'dd', 'ol', 'ul',
	...
	htmlTags : {
		...
		hr : ['/'],
		br : ['/'],
		'dd,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6' : [
...
KE.queryCommandValue = function(doc, cmd) {
	...
	if (cmd === 'fontname') {
		...
		if (val === '') {
			...
			if (!el) el = KE.getCommonAncestor(keSel, 'dd');
			if (el) val = el.nodeName;
		}
		if (val === 'Normal') val = 'dd';
...
KE.util = {
	...
	toData : function(id, srcData) {
		...
		html = html.replace(/^\s*<dd>\s*&nbsp;\s*<\/dd>\s*$/ig, '');
		if (g.filterMode) {
	...
	pasteHtml : function(id, html, isStart) {
		...
		////////////////////////////////////////////////////
			if(!/.*<br\s*\/>$/g.test(g.iframeDoc.body.innerHTML)){
				g.iframeDoc.body.innerHTML += "<br />";//添加一个换行
			};
		////////////////////////////////////////////////////
	...
	addNewlineEvent : function(id) {
		...
		if (KE.browser.GECKO && KE.browser.VERSION < 3 && g.newlineTag.toLowerCase() != 'dd') return;
		...
		KE.event.add(g.iframeDoc, 'keydown', function(e) {
			...
			if (g.newlineTag.toLowerCase() == 'br') {
				...
				if (!KE.util.inArray(tagName, ['dd', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'pre', 'div', 'li'])) {
...
KE.plugin['fullscreen'] = {
	click : function(id) {
		var g = KE.g[id];
	////////////////////////////////
		var parContaint,parMain;//控制全屏
		if(g.parObj==undefined){
			var parWin = g.iframeWin.top;
			var parDoc = parWin.document;
			var $ = parWin.$;
			parContaint = $("body .container:first",parDoc);
			parMain = parContaint.find(".main:first");
			g.parObj = {
				"parContaint" : parContaint,
				"parMain" : parMain
			};
		}else{
			parContaint =  g.parObj.parContaint;
			parMain = g.parObj.parMain;
		}
	///////////////////////////////
		var self = this;
		...
		if (this.isSelected) {
			...
			parMain.css({position : 'absolute',
	    				width : parContaint.width(),
	    				height: parContaint.height(),
	    				left : 0,top : 20,zIndex : 29811211});
...
KE.plugin['image'] = {
	...
	exec : function(id) {
		...
		if (type == 2) {
			...
			var resize = KE.$('resize', dialogDoc);
			resize.value = resize.checked ? true : false;
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						self.insert(id, data.url, title, width, height, 0, align, resize.checked);
					} else {
						alert(data.message);
						return false;
					}
	...
	insert : function(id, url, title, width, height, border, align, resize) {
		var html = '<img src="' + url + '" kesrc="' + url + '" ';
		if (resize) html += 'class="point" ';
